# pylint: skip-file
import copy as _copy, json, pickle, tempfile, unittest
from collections import abc

import orjson
from epicstuff import Copy, NewDict as Dict
...
class Main(unittest.TestCase):
	...
	# =========================================================================
	def test_01_print_no_recursion(self) -> None:
	...
	#    dict() + reversed().
	# =========================================================================
	def test_02_dict_ops_all_convert_modes(self) -> None:
	...
	# =========================================================================
	def test_03_subclassing(self) -> None:
	...
	# =========================================================================
	def test_05_convert_flag_stored_as_passed(self) -> None:
	...
	# =========================================================================
	def test_06_lazy_convert_one_level(self) -> None:
		d = Dict({'a': {'b': {'c': 1}}})
	...
	# =========================================================================
	def test_07_nested_plain_dict_is_newdict(self) -> None:
		class tmp(dict): ...
		a = Dict(tmp({'a': {'b': 1}}))
	...
	# =========================================================================
	def test_09_multiple_inheritance(self) -> None:
	...
	# =========================================================================
	def test_10_pickle_roundtrip(self) -> None:
		x = [
	...
	# =========================================================================
	def test_11_or_merge(self) -> None:
		x = Dict({'a': 1, 'b': 2})
	...
	# =========================================================================
	def test_12_hasattr_missing(self) -> None:
		x = Dict({'a': 1, 'b': 2})
	...
	# =========================================================================
	def test_13_or_precedence(self) -> None:
		a = Dict(a=1)
		b = Dict(a=2)
		assert (a | b).a == 2
		assert (b | a).a == 1
	...
	# =========================================================================
	def test_14_empty_is_dict_and_mapping(self) -> None:
		x = Dict()
	...
	# =========================================================================
	def test_15_values_unpackable(self) -> None:
		_a, _b = Dict({'a': {}, 'b': {}}).values()

	# =========================================================================
	# 17. _convert=None: mutations through dot-accessed nested mappings propagate.
	# =========================================================================
	def test_17_mutation_propagation(self) -> None:
	...
	# =========================================================================
	def test_18_create_true_autovivify(self) -> None:
		d = Dict(_create=True)
		d.a.b.c = 1
		assert d.a.b.c == 1
		assert type(d.a) is Dict
	...
	# =========================================================================
	def test_19_create_custom_callable(self) -> None:
	...
	# =========================================================================
	def test_20_missing_without_create(self) -> None:
		d = Dict({'a': 1})
		try:
			_ = d['nonexistent']
	...
	# =========================================================================
	def test_21_in_no_side_effect(self) -> None:
		d = Dict(_create=True)
	...
	# =========================================================================
	def test_22_hasattr_getattr_suppress_create(self) -> None:
		d = Dict(_create=True)
	...
	# =========================================================================
	def test_24_copy_deepcopy(self) -> None:
	...
	# =========================================================================
	def test_25_update_converts(self) -> None:
	...
	# =========================================================================
	def test_26_convert_and_cache(self) -> None:
	...
	# =========================================================================
	def test_27_no_double_wrap(self) -> None:
	...
	# =========================================================================
	def test_28_get_no_autovivify(self) -> None:
		d = Dict(_create=True)
	...
	# =========================================================================
	def test_29_commentedmap(self) -> None:
	...
	# =========================================================================
	def test_30_method_conversion_matrix(self) -> None:
	...
	def test_31_method_semantics(self) -> None:
	...
	# =========================================================================
	def test_32_flag_inherit_and_override(self) -> None:
		def conv(m): return Dict(m)
		...
		z = Dict(x)
		assert z._s.convert is True
		assert z._s.converter is conv
		assert z._s.create is x._s.create
		assert z._s.create
		...
		f = Dict({'a': 1})
		assert f._s.convert is None
		assert f._s.create is False

	# =========================================================================
	# 35. pickle preserves the wrapped source class.
	# =========================================================================
	def test_35_pickle_preserves_subclass(self) -> None:
		d = Dict(_Special({'a': 1}))
	...
	# =========================================================================
	def test_36_pickle_preserves_commentedmap(self) -> None:
		cm = CommentedMap({'a': 1, 'b': 2})
		cm.yaml_set_comment_before_after_key('a', before='hello')
		d = Dict(cm)
	...
	# =========================================================================
	def test_37_copy_preserves_commentedmap(self) -> None:
		cm = CommentedMap({'a': 1, 'b': 2})
		cm.yaml_set_comment_before_after_key('a', before='hello')
		d = Dict(cm)
	...
	# =========================================================================
	def test_38_copy_independent_create(self) -> None:
		d = Dict(_create=True)
		c = d.copy()
	...
	# =========================================================================
	def test_39_copy_function(self) -> None:
		...
		assert c == cm
		assert c is not cm
		assert isinstance(c, CommentedMap)
		assert c.ca.items
	...
	# =========================================================================
	def test_40_copy_false_unsupported_source(self) -> None:
	...
	#
	#     NOT REGISTERED in the original harness (was commented out of TESTS), so
	#     it is preserved here as a non-test helper (leading underscore keeps
	#     unittest from collecting it) to retain exact run semantics.
	# =========================================================================
	def _test_41_self_referential(self) -> None:
	...
	# =========================================================================
	def test_42_okay_private_keys_autovivify(self) -> None:
		class WithOkay(Dict):
		...
		val = d._ok_
		assert isinstance(val, WithOkay)
		assert len(val) == 0
		assert '_ok_' in d
	...
	# =========================================================================
	def test_43_nested_mapping_identity_stable(self) -> None:
	...
	# =========================================================================
	def test_44_settings_object(self) -> None:
		def conv(m): return Dict(m)
		def cr(k) -> str: return f'v-{k}'
		...
		assert s.convert is True
		assert s['_convert'] is True
		assert s.create is True
		assert s['_create'] is True
		assert 'convert' in s
		assert '_convert' in s
		assert 'create' in s
		assert '_create' in s

		# an explicitly-set converter/creater is returned verbatim (key over attr)
		assert s.converter is conv
		assert s['_converter'] is conv
		...
		assert f._s.convert is None
		assert f._s.create is False
		assert 'converter' not in f._s
		assert 'creater' not in f._s
		...
		f._s.convert = True
		assert f._s['_convert'] is True
		assert f._s.convert is True
		assert 'convert' in f._s
	...
	# =========================================================================
	def test_45_or_consistency(self) -> None:
		...
		assert r._s.convert is True
		assert type(r.b) is Dict
		assert r.b.c == 1
		assert r.a == 1
		...
		r = F | {'y': {'k': 1}}
		assert type(r) is Dict
		assert type(r) is not dict
		assert r._s.convert is False
	...
	# =========================================================================
	# 46. Make sure it works with json/orjson, mostly for nicegui.
	# =========================================================================
	...
	def test_46_json(self, convert, create) -> None:
		data = {
			'name': 'Ada',
			'nested': {'count': 2},
			'items': [{'active': True}],
		...
		value = Dict(data, _convert=convert, _create=create)

		# NewDict and recursively converted children remain serializable as dicts.
